...
def main():
    ...
    #ps['pitch']=-ps['pitch'] 
        ...
        if cnt%frame_ratio==0:
            ...
            simExp = -1
            if mono:
                zmq_pub.send_multipart([zmq_topics.topic_stereo_camera,pickle.dumps([ frame_cnt, (imgl.shape[0]*2,imgl.shape[1]*2, 3 ), time.time(), simExp , False]),imgl.tobytes(), b''])
            else:
                zmq_pub.send_multipart([zmq_topics.topic_stereo_camera,pickle.dumps([frame_cnt,(imgl.shape[0]*2,imgl.shape[1]*2, 3 ), time.time(), simExp , False]),imgl.tobytes(), b'' , imgr.tobytes(), b'' ] )
            time.sleep(0.001) 
